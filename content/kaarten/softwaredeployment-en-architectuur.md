---
title: Softwaredeployment & Architectuur
tags: [kaart, moc, softwaredeployment-en-architectuur]
---

# Softwaredeployment & Architectuur

Deze kaart biedt een overzicht van alle begrippen binnen **softwaredeployment & architectuur**.

## Begrippen (23 items)

- [[on-premises-software|On‑premises software]]
- [[cloud-native-applicatie|Cloud‑native applicatie]]
- [[hybride-cloud|Hybride cloud]]
- [[multi-cloud|Multi‑cloud]]
- [[software-as-a-service|Software as a Service (SaaS)]]
- [[platform-as-a-service|Platform as a Service (PaaS)]]
- [[infrastructure-as-a-service|Infrastructure as a Service (IaaS)]]
- [[containertechnologie|Containertechnologie]]
- [[docker-container|Docker‑container]]
- [[docker-compose|Docker Compose]]

- [[portainer|Portainer (containerbeheer)]]
- [[kubernetes|Kubernetes (K8s)]]
- [[helm-charts|Helm‑charts]]
- [[red-hat-openshift|Red Hat OpenShift]]
- [[container-registry|Container‑registry]]
- [[ci-cd-pijplijn|CI/CD‑pijplijn]]
- [[gitops|GitOps]]
- [[api-gateway|API‑gateway]]
- [[versiebeheer-met-git|Versiebeheer met Git]]

## UNS & Architectuurprincipes

**Nieuw toegevoegd - Unified Namespace en moderne architecturen:**
- [[unified-namespace|Unified Namespace (UNS)]] - ⭐ **Centraal architectuurconcept**
- [[single-source-of-truth|Single Source of Truth (SSOT)]]
- [[ontkoppeling-van-applicaties|Ontkoppeling van applicaties]]
- [[isa-95-model|ISA-95 model]]

## Gerelateerde kaarten

**Technische implementatie:**
- [[kaarten/event-streaming-messaging-en-integratie|Event streaming, messaging & integratie]] - Event-gedreven UNS architectuur
- [[kaarten/industriële-communicatieprotocollen|Industriële communicatieprotocollen]] - MQTT en OPC UA voor UNS
- [[kaarten/data-analyse-en-business-intelligence|Data & Business Intelligence]] - UNS data analytics

**Toegepaste scenario's:**
- [[kaarten/digitale-transformatie-en-industrie-40-50|Digitale transformatie]] - UNS als enabler voor Industrie 4.0
- [[kaarten/softwaremodules-en-integratie|Softwaremodules & Integratie]] - UNS integratie met bestaande systemen

---
← Terug naar [[kaarten/index|Overzicht van alle kaarten]] | [[index|Kompas hoofdpagina]]
