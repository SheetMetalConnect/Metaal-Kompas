---
tags:
  - 'kaart'
  - 'moc'
  - 'data-analyse-en-business-intelligence'
  - '🧹draft'

title: Data, Analyse & Business Intelligence
---
Deze kaart biedt een overzicht van alle begrippen binnen **data, analyse & business intelligence**.

## Begrippen 

- [[structured-query-language|Structured Query Language (SQL)]]
- [[nosql-databases|NoSQL‑databases]]
- [[datawarehouse|Datawarehouse]]
- [[datalake|Datalake]]
- [[tijdreeksdatabase|Tijdreeksdatabase (time‑series database)]]
- [[historian-database|Historian‑database (proceshistorian)]]
- [[master-data-management|Master Data Management (MDM)]]
- [[datagovernance|Datagovernance]]
- [[business-intelligence|Business Intelligence (BI)]]
- [[kpi-dashboard|KPI‑dashboard]]
- [[data-mining|Data‑mining]]
- [[extract-transform-load|Extract‑Transform‑Load (ETL)]]
- [[predictive-analytics|Predictive analytics (voorspellende analyse)]]
- [[prescriptive-analytics|Prescriptive analytics (voorschrijvende analyse)]]
- [[online-analytical-processing|Online Analytical Processing (OLAP)]]
- [[data-herkomst|Data‑herkomst (data lineage)]]
- [[metadata-beheer|Metadata‑beheer]]

## UNS Data Management

**Data concepten binnen Unified Namespace:**
- [[contextualisatie-van-data|Contextualisatie van data]] - ⭐ **Data verrijking voor UNS**
- [[semantisch-datamodel|Semantisch datamodel]] - **Betekenis binnen UNS**
- [[data-democratisering|Data democratisering]] - **Self-service toegang tot UNS data**

## Gerelateerde kaarten

**UNS Foundation:**
- [[kaarten/softwaredeployment-en-architectuur|Softwaredeployment & Architectuur]] - UNS architecture & SSOT
- [[kaarten/event-streaming-messaging-en-integratie|Event streaming]] - Real-time UNS data flows

**Technical Implementation:**
- [[kaarten/data-collectie-en-industrial-data-platforms|Data-collectie]] - UNS data sources
- [[kaarten/industriële-communicatieprotocollen|Communicatieprotocollen]] - UNS data protocols

**Digital Transformation:**
- [[kaarten/digitale-transformatie-en-industrie-40|Digitale transformatie]] - UNS-enabled analytics

---
← Terug naar [[kaarten/index|Overzicht van alle kaarten]] | [[index|Kompas hoofdpagina]]
