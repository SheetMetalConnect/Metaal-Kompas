---
title: Programmatie & Aansturing Productiemiddelen
tags: [kaart, moc, programmatie-en-aansturing-productiemiddelen]
---


Deze kaart biedt een overzicht van alle begrippen binnen **programmatie & aansturing productiemiddelen**.

## Begrippen (16 items)

- [[cnc-programmatie|CNC‑programmatie]]
- [[g-code|G‑code]]
- [[m-code|M‑code]]
- [[cam-postprocessor|CAM‑postprocessor]]
- [[offline-programmeren|Offline programmeren (OLP)]]
- [[plc-programmering|PLC‑programmering]]
- [[ladder-diagram|Ladder Diagram (LD)]]
- [[function-block-diagram|Function Block Diagram (FBD)]]
- [[structured-text|Structured Text (ST)]]
- [[robot-teach-pendant|Robot‑teach pendant]]

- [[robot-padplanning|Robot‑padplanning (path planning)]]
- [[roboticasimulatiesoftware|Roboticasimulatiesoftware]]
- [[lasrobot-programmering|Lasrobot‑programmering]]
- [[wegpunt-programmering|Wegpunt‑programmering (waypoints)]]
- [[kinematica-in-robotica|Kinematica in robotica]]
- [[adaptieve-lassoftware|Adaptieve lassoftware (adaptive welding)]]


## Gerelateerde kaarten

[PLACEHOLDER - Links naar gerelateerde kaarten worden later toegevoegd]

---
← Terug naar [[kaarten/index|Overzicht van alle kaarten]] | [[index|Kompas hoofdpagina]]
